...
(function(){
            ...
            angular.extend($scope, {
                ...
                greenMtWest: {
                    ...
                    label: {
                        message: 'Green Mountain Meeting Spot (west)',
                        show: false,
                        showOnMouseOver: true
                ...
                controls: [
            	 ...
            	 layers: [
		            	...
		            	{
		                	name: '404-rides',
		                	label: '404 Group Rides',
		                	active: true,
		                	source: {
		                    type: 'GeoJSON',
		                    url: 'data/group-rides.geojson'
		                	},
		                	style: {
		                    fill: {
		                        color: 'rgba(255, 0, 255, 0.6)'
	...
	/*			var highlightStyleCache = {};
            ...
            $scope.$on('openlayers.layers.routes.mousemove', function(event, feature) {
         	...
         	$scope.$on('openlayers.layers.404-rides.mousemove', function(event, feature) {
                 $scope.$apply(function(scope) {      					
		                if(feature && $scope.rides[feature.getProperties().id]) {
		                	//feature.setStyle(highlightStyle);
		                	
		                	$("#fixed-hover-window").show();
		                	
		                  $scope.mouseMoveRoute = feature?$scope.rides[feature.getProperties().id].properties.name:'';
		                  if(!$scope.mouseMoveRoute){
		                  	$scope.mouseMoveRoute = 'n/a';
		                  }
		                }          
           ...
           // Get the routes data and append it to the $scope variable
	        ...
	        $http.get("data/group-rides.geojson").success(function(data, status) {
	            // Put the countries on an associative array
	            $scope.rides = {};
	            for (var i=0; i< data.features.length; i++) {
	                var route = data.features[i];
	                $scope.rides[data.features[i].properties.id] = route;
	            }
	        });
